...
; CALLING SEQUENCE:
;
;       Poly_Surface, vertices, polygons
;
; REQUIRED INPUTS:
;
...
; OPTIONAL INPUTS
;
;       vertices: A 3-by-n array containing the vertices of the polygon, as
;                 returned by Shade_Volume.
;
;       polygons: A polygon list of the sort returned from Shade_Volume.
;
...
PRO Poly_Surface_Draw_Events, event
...
CASE newStyle OF
   ...
   'DOTS': info.thisPolygon->SetProperty, Style=0
   'MESH': info.thisPolygon->SetProperty, Style=1
   'SOLID': info.thisPolygon->SetProperty, Style=2, Shading=1
   'HIDDEN': BEGIN
       ...
       info.thisPolygon->SetProperty, Hidden_Lines=setting
       ENDCASE
...
PRO Poly_Surface_Printing, event
...
IF result EQ 1 THEN BEGIN
   ...
   info.thisPolygon->GetProperty, Color=surfaceColor
   ...
   info.thisPolygon->SetProperty, Color=[70,70,70]
   ...
   info.thisPolygon->SetProperty, Color=surfaceColor
...
PRO Poly_Surface, vertices, polygons, _Extra=extra, XTitle=xtitle, $
...
IF N_Elements(vertices) EQ 0 OR N_Elements(polygons) EQ 0 THEN BEGIN
...
thisPolygon = OBJ_NEW('IDLgrPolygon', vertices[0,*], vertices[1,*], vertices[2,*], $
   Polygons=polygons, Color=[255,255,0], Shading=1)

    ; Get the data ranges of the surface.

thisPolygon->GetProperty, XRange=xrange, YRange=yrange, ZRange=zrange
...
thisPolygon->SetProperty, XCoord_Conv=xs, YCoord_Conv=ys, ZCoord_Conv=zs

    ; Add the surface and axes objects to the model.

thisModel->Add, thisPolygon
thisModel->Add, xAxis
thisModel->Add, yAxis
thisModel->Add, zAxis
...
drawID = Widget_Draw(tlb, XSize=400, YSize=400, Graphics_Level=2, Retain=0, $
   Expose_Events=1, Event_Pro='Poly_Surface_Draw_Events', Button_Events=1)
...
dummy = Widget_Button(outputID, Value='GIF File', $
   UValue='GIF', Event_Pro='Poly_Surface_Output')
dummy = Widget_Button(outputID, Value='JPEG File', $
   UValue='JPEG', Event_Pro='Poly_Surface_Output')

dummy = Widget_Button(fileID, Value='Print', Event_Pro='Poly_Surface_Printing')

dummy = Widget_Button(fileID, /Separator, Value='Exit', $
   Event_Pro='Poly_Surface_Exit')

   ; Create STYLE menu buttons for surface style.

styleID = Widget_Button(menubase, Value='Style', Event_Pro='Poly_Surface_Style', /Menu)
...
info = { thisContainer:thisContainer, $ ; The object container.
         ...
         thisPolygon:thisPolygon, $     ; The surface object.
...
XManager, 'Poly_Surface', tlb, Cleanup='Poly_Surface_Cleanup', /No_Block, $
   Event_Handler='Poly_Surface_Resize', Group_Leader=groupLeader
END
;-------------------------------------------------------------------